...
class MainWindow(QMainWindow):
    ...
    def game_play(self, board_hard, init_hard, board_easy, init_easy):
        self.board_hard = board_hard
        self.init_hard = init_hard
        self.board_easy = board_easy
        self.init_easy = init_easy
        self.reset_hard()
    ...
    def reset_hard(self):
        ...
        button_play_hard = QPushButton("Solve Hard Sudoku", self)
        button_play_hard.setGeometry(850, 80, 200, 50)
        button_play_hard.clicked.connect(self.show_board_hard)
        button_reset_hard = QPushButton("Reset Hard", self)
        button_reset_hard.setGeometry(850, 132, 200, 50)
        button_reset_hard.clicked.connect(self.reset_hard)

        button_play_easy = QPushButton("Solve Easy Sudoku", self)
        button_play_easy.setGeometry(850, 185, 200, 50)
        button_play_easy.clicked.connect(self.show_board_easy)
        button_reset_easy = QPushButton("Reset Easy", self)
        button_reset_easy.setGeometry(850, 238, 200, 50)
        button_reset_easy.clicked.connect(self.reset_easy)
        y_axis = -30
        for x in range(9):
            x_axis = 20
            y_axis += 80
            for y in range(9):
                if self.init_hard[x][y] == 0:
                    button = QPushButton("", self)
                    button.setGeometry(x_axis, y_axis, 83, 83)
                    button.setStyleSheet(u"background-color: #ECF0F1")

                else:
                    button = QPushButton(str(self.init_hard[x][y]), self)
                    button.setGeometry(x_axis, y_axis, 83, 83)
                    button.setStyleSheet(u"background-color: lightblue")
                    button.setFont(QFont("Roboto", 17))
                x_axis += 80
        self.show()
    def show_board_easy(self):
        self.hide()
        y_axis = -30
        for x in range(9):
            x_axis = 20
            y_axis += 80
            for y in range(9):
                if self.board_easy[x][y] == 0:
                    button = QPushButton("", self)
                    button.setGeometry(x_axis, y_axis, 83, 83)
                else:
                    button = QPushButton(str(self.board_easy[x][y]), self)
                    button.setGeometry(x_axis, y_axis, 83, 83)
                    button.setStyleSheet(u"background-color: #EBF5FB; color: #5D6D7E")

                    button.setFont(QFont("Roboto", 17))
                x_axis += 80
        self.show()
    def show_board_hard(self):
        self.hide()
        y_axis = -30
        for x in range(9):
            x_axis = 20
            y_axis += 80
            for y in range(9):
                if self.board_hard[x][y] == 0:
                    button = QPushButton("", self)
                    button.setGeometry(x_axis, y_axis, 83, 83)
                else:
                    button = QPushButton(str(self.board_hard[x][y]), self)
                    button.setGeometry(x_axis, y_axis, 83, 83)
...
class Data:
    def __init__(self):
        # The init ata Bai tap 1 in week11Exercises
        self.init_assign_bai1 = '.2...194881.6.......4.276..17..9...33.........48.53.....61....2....74..6.5.9....7'
        # The init ata Bai tap 2 in week11Exercises
        self.init_assign_bai2 = '.9.............46.2.......5.7.8.4.1..3..2....5...6.8...1.73..2.9.8.5.......2.....'
        # The init data hard sudoku in source of Teacher's Quang
        self.init_assign_easy = '..3.2.6..9..3.5..1..18.64....81.29..7.......8..67.82....26.95..8..2.3..9..5.1.3..'
        # The init data hard sudoku https://sudoku.com/hard/
        self.init_assign_hard = '.........6...3...4..25....83....58.77..1.....8..2....6..6..1.4...94....5......17.'
        # The init data expert sudoku https://sudoku.com/expert/
        self.init_assign_expert = '.....4..7.2..1.95.6..2.......68...1..5.1..4.82.....6......7....18.........4.6..9.'
        # The init data expert sudoku https://sudoku.com/evil/
        self.init_assign_evil = '..4......2.5..18.....8....3.9...........7..6.1.8..53...3...9....4....2..9.2.5...7'
# %% main
if __name__ == '__main__':
    ...
    # Easy
    sudoku = Sudoku(data.init_assign_bai1)
    initAC3 = de_init_board(data.init_assign_bai1)
    print('VARIABLES:')
    ...
    sudoku.display()
    solution_easy = boards(sudoku.domains)

    # ========================================================================

    print('\nHARD SUDOKU:')
    sudoku.display()
    sudoku1 = Sudoku(data.init_assign_bai2)
    # get init board
    init = de_init_board(data.init_assign_bai2)
    ...
    window.game_play(solution, init, solution_easy, initAC3)
    sys.exit(app.exec())
